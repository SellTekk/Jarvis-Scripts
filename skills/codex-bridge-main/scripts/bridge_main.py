...
def create_issue(repo: str, token: str, branch: str, task: str):
    ...
    st, data = gh_api("POST", f"/repos/{repo}/issues", token, body={"title": title, "body": body, "labels": ["codex", "automation"]})
    if st not in (200, 201):
...
def dispatch(args):
    ...
    if not status.stdout.strip():
        ...
        try:
            ...
            api_push_changed_files(repo, token, branch, local_path, f"[codex] seed: {task[:60]}")
    pr_url, pr_no = create_pr(repo, token, head=branch, base=base, title=f"[Codex] {task[:80]}", body=f"Task: {task}\nBranch: {branch}\n", labels=["codex", "automation"])
    comment_issue(repo, token, pr_no, f"@codex {task}\n\nPlease commit your changes in this PR.")
...
def sync(args):
    ...
    log(f"sync done, merged {merged} PRs")
