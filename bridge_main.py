...
def create_pr(repo: str, token: str, head: str, base: str, title: str, body: str, draft: bool = False, labels: list = None):
    ...
    # PR Labels laufen über Issues API
    if labels:
        add_issue_labels(repo, token, pr_no, labels)
...
def dispatch(args):
    ...
    # Clone if needed - first ensure directory is clean
    # print(f'Cleaning local path: {local_path}')
    ...
    #         # Decide how to handle this error - maybe re-raise or continue cautiously
    #         raise e # Re-raising to stop if cleanup fails
...
def sync(args):
    ...
    for pr in prs:
        ...
        if pr_branch.startswith("codex-"):
            ...
            # Check labels before merging - ONLY merge if codex-automerge label is set
            if merge:
